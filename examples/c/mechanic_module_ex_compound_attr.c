/**
 * Compound datatypes with compound attributes
 * ===========================================
 *
 * In this example, we use H5T_COMPOUND data type for use with structures, both for
 ...
 *        mechanic_module_ex_compound_attr.c -o libmechanic_module_ex_compound_attr.so
 *
 * Using the module
 * ----------------
 *
 *    mpirun -np 4 mechanic -p ex_compound_attr -x 10 -y 20
 *
 * Getting the data
 * ----------------
 *
...
int Storage(pool *p) {
  ...
  // Attr 0 for the pool bank 0
  p->storage[0].attr[0].layout = (schema) {
    .name = "pool-sensors-env",
    .dataspace = H5S_SCALAR,
    .datatype = H5T_COMPOUND,
    .compound_size = sizeof(env_t),
  };
  
  // Field definition for the attr 0 of the pool bank 0
  p->storage[0].attr[0].field[0].layout = 
    (schema) {.name = "lab", .datatype = H5T_NATIVE_CHAR,
    .rank = 1, .dims = {257}, .field_offset = HOFFSET(env_t, lab)};
  
  p->storage[0].attr[0].field[1].layout = 
    (schema) {.name = "temperature", .datatype = H5T_NATIVE_DOUBLE,
    .rank = 1, .dims = {1}, .field_offset = HOFFSET(env_t, temperature)};

  p->storage[0].attr[0].field[2].layout = 
    (schema) {.name = "pressure", .datatype = H5T_NATIVE_DOUBLE,
    .rank = 1, .dims = {1}, .field_offset = HOFFSET(env_t, pressure)};
...
int PoolPrepare(pool **all, pool *p) {
  ...
  sprintf(env.lab, "Laboratory 404");
  env.temperature = 23.13;
  env.pressure = 1012.14;

  MWriteData(p, "pool-sensors", &sensors[0][0]);
  MWriteData(p, "pool-particles", &particles[0][0]);
  MWriteAttr(p, "pool-sensors", "pool-sensors-env", &env);
