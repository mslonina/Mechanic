...
def detect(conf):
	try: 
		...
		raise Configure.ConfigurationError(
				"Add set_options(opt): opt.tool_options('compiler_mpif90')")
...
def set_options(opt):
	...
	fortran_compiler_opts = opt.add_option_group("MPIF90 Compiler Options")
	fortran_compiler_opts.add_option('--check-mpif90-compiler', 
			default="%s" % test_for_compiler,
			help='On this platform (%s) the following MPIF90 Compiler will be checked by default: "%s"' % (detected_platform, test_for_compiler),
		dest="check_fortran_compiler")

	for compiler in test_for_compiler.split():
		opt.tool_options('%s' % compiler, option_group=fortran_compiler_opts, tooldir=tooldir)
